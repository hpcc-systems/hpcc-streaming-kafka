...
EXPORT telematics_calc(STRING filename) := FUNCTION
	...
	$.layouts.base calcDist($.layouts.base le, $.layouts.base ri) := TRANSFORM
		...
		calc_dist := IF(le.lat = 0.0 or le.lon = 0.0, 0.0, $.haversine_dist(le.lat, le.lon,	ri.lat, ri.lon));
		...
		SELF.speed_cat := MAP(speed_r BETWEEN by_s.range1_lo AND by_s.range1_hi => 1,				
