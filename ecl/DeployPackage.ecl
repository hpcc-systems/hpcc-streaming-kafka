...
DeployedQueries := FUNCTION
	...
	response := SOAPCALL(	requestDataset, 
...
telematicsqueries := DeployedQueries(query_name[1..10] = 'telematics'); // Filter out the queries for telematics project
...
getSuperFile(String queryname) := FUNCTION
	mapOfQueries := MAP((queryname = 'telematics_service_accdec') => $.files.SUPERKEY_ACCDEC,
...
pack_for_query1 := $.CreatePackageMapString( ds_results[1].queryName, 
																					  ds_results[1].superfile, 
																						STD.Str.FindReplace(ds_results[1].superfile, '~', ''));
																						
pack_for_query2 := $.CreatePackageMapString( ds_results[2].queryName, 
